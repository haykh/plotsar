...
def PulsarCard(pulsar):
    ...
    if pulsar["Catalog"] == "ATNF":
        ...
        xc, yc = np.sqrt(xedges[:-1] * xedges[1:]), np.array(pulsar["nuFnu_Band"])
        xs, ys = (
        ...
        ymax = np.ceil(np.log10(ys.max()) * 2) / 2
        ymin = np.floor(np.log10((yc - yerr_dwn)[~np.isnan(yerr_dwn)]).min())
        ymin = min(ymax - 2, ymin)
        fig = go.Figure()
        fig.add_trace(
            go.Scatter(
                x=xs,
                y=ys,
                mode="lines",
                line_color=plotly.colors.DEFAULT_PLOTLY_COLORS[3],
            )
        )
        fig.add_trace(
            go.Scatter(
                x=xc,
                y=yc,
                marker_color=plotly.colors.DEFAULT_PLOTLY_COLORS[0],
                mode="markers",
                error_y=dict(
                    type="data",
                    symmetric=False,
        ...
        spectrum = dash.dcc.Graph(
            figure=fig, id="spectrum", config={"displaylogo": False, **save_config}
        )
    return [
